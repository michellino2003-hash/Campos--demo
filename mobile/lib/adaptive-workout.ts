...
export type GuidedStep = { label: string; dose: string; instruction: string; rest?: string };
export type AdaptiveWorkoutBlock = { title: string; detail: string; duration: number; cue: string; type: 'prep' | 'boxing' | 'strength' | 'recovery'; steps: GuidedStep[] };
export type LiveSessionFeedback = { effort: 1 | 2 | 3 | 4 | 5; technique: 'sharp' | 'slipping' | 'breaking-down'; pain: 'none' | 'mild' | 'rising'; breathing: 'controlled' | 'working' | 'struggling' };
export type LiveCoachAdjustment = { mode: 'continue' | 'ease' | 'recover' | 'stop'; message: string; targetRpeDelta: number; extraRestSeconds: number; durationMultiplier: number; powerCeiling?: string };

type ProfiledPrescription = SessionPrescription & { athleteProfile?: AthleteProfile };

const BASE: AdaptiveWorkoutBlock[] = [
  { title: 'Movement prep', detail: 'Boxing mobility + warm-up', duration: 480, cue: 'Move smoothly. Prepare the positions you will use today.', type: 'prep', steps: [
  ...
  { title: 'Shadowboxing', detail: '3 × 2-minute technical rounds', duration: 540, cue: 'Stay light and finish every combination balanced.', type: 'boxing', steps: [
  ...
  { title: 'Heavy bag', detail: '3 controlled precision rounds', duration: 540, cue: 'Snap shots, keep your feet under you, and never admire your work.', type: 'boxing', steps: [
  ...
  { title: 'Strength block', detail: 'Press, row, rear delts', duration: 1440, cue: 'Own every rep. Keep one to three clean reps in reserve.', type: 'strength', steps: [
  ...
  { title: 'Cooldown', detail: 'Grip, mobility and downshift breathing', duration: 720, cue: 'Finish calmer than you started.', type: 'recovery', steps: [
...
function applyEquipment(blocks: AdaptiveWorkoutBlock[], profile?: AthleteProfile | null) {
  ...
  if (!profile.equipment.includes('heavy-bag')) {
    next = next.map((block) => block.title === 'Heavy bag' ? { ...block, title: 'Shadowboxing pressure rounds', detail: '3 controlled rounds · no bag required', cue: 'Create imaginary targets, change distance, and finish every exchange balanced.', steps: block.steps.map((step) => ({ ...step, label: step.label.replace('Jab control', 'Jab + angle').replace('Straight shots', 'Straight shots + defense'), instruction: `${step.instruction} Work against an imaginary opponent and prioritize clean positioning over power.` })) } : block);
  }
  if (!profile.equipment.includes('gym')) {
    const hasDumbbells = profile.equipment.includes('dumbbells');
    next = next.map((block) => block.type === 'strength' ? { ...block, detail: hasDumbbells ? 'Dumbbell strength · no gym required' : 'Bodyweight strength · no gym required', steps: hasDumbbells ? [
...
export function buildAdaptiveWorkout(prescription?: SessionPrescription | null, profile?: AthleteProfile | null): AdaptiveWorkoutBlock[] {
  const embeddedProfile = (prescription as ProfiledPrescription | null | undefined)?.athleteProfile;
  const resolvedProfile = profile ?? embeddedProfile;
  let blocks = BASE;
  if (prescription?.stopForPain) {
    blocks = [{ ...BASE[0], duration: 360, detail: 'Pain-free mobility only', cue: 'Nothing should increase pain.' }, { ...BASE[4], duration: 600, detail: 'Recovery only', steps: BASE[4].steps.filter((step) => step.label !== 'Suitcase hold') }];
  } else if (prescription?.mode === 'recover') {
    blocks = [{ ...BASE[0], duration: 420 }, { ...BASE[1], duration: 360, detail: '2 relaxed technical rounds', steps: BASE[1].steps.slice(0, 2) }, { ...BASE[2], duration: 300, detail: '2 light precision rounds', steps: BASE[2].steps.slice(0, 2) }, BASE[4]];
  } else if (prescription && prescription.volumeMultiplier < 1) {
    blocks = BASE.map((block) => block.type === 'strength' ? { ...block, duration: Math.round(block.duration * prescription.volumeMultiplier), detail: 'Reduced-volume strength · pain-free range' } : block.type === 'boxing' ? { ...block, duration: Math.round(block.duration * 0.85), detail: `${block.detail} · technical pace` } : block);
  ...
  return personalize(blocks, resolvedProfile);
}

export function getLiveCoachAdjustment(feedback: LiveSessionFeedback): LiveCoachAdjustment {
  if (feedback.pain === 'rising') return { mode: 'stop', message: 'Pain is rising. Loaded work is done for today; protect the next training day.', targetRpeDelta: -3, extraRestSeconds: 120, durationMultiplier: 0.35, powerCeiling: 'No power work' };
  if (feedback.technique === 'breaking-down' || feedback.breathing === 'struggling' || feedback.effort === 5) return { mode: 'recover', message: 'Quality is dropping. CampOS is cutting volume before fatigue turns into bad reps.', targetRpeDelta: -2, extraRestSeconds: 60, durationMultiplier: 0.65, powerCeiling: '50–60%' };
  if (feedback.pain === 'mild' || feedback.technique === 'slipping' || feedback.effort === 4) return { mode: 'ease', message: 'Stay productive, but sharpen the work: more recovery, less volume, clean technique only.', targetRpeDelta: -1, extraRestSeconds: 30, durationMultiplier: 0.82, powerCeiling: '60–70%' };
...
export function adaptRemainingWorkout(blocks: AdaptiveWorkoutBlock[], completedThroughIndex: number, feedback: LiveSessionFeedback): { blocks: AdaptiveWorkoutBlock[]; adjustment: LiveCoachAdjustment } {
  ...
  const adapted = blocks.map((block, index) => {
    ...
    return { ...block, duration: Math.max(120, Math.round(block.duration * adjustment.durationMultiplier)), detail: `${block.detail} · live adjusted`, cue: adjustment.message, steps: block.steps.slice(0, keepSteps).map((step) => ({ ...step, instruction: `${step.instruction} ${adjustment.powerCeiling ? `Cap power at ${adjustment.powerCeiling}. ` : ''}${adjustment.extraRestSeconds ? `Take up to ${adjustment.extraRestSeconds} extra seconds before repeating quality work.` : ''}`.trim() })) };
